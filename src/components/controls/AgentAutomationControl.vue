...
function buildPayloads() {
  // Force 1 image only when using Agent (as requested for "văn bản thành hình ảnh" flow)
  const outputCount = 1;
  return {
    payloads: prompts.value.map((prompt, index) => ({
      ...
      imageOption: getPromptOption(index, {
        defaultPromptOption: props.settings.defaultImageOption,
